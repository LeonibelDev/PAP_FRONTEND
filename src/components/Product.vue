    ...
    <div id="carousel1_indicator" class="carousel slide" data-ride="carousel" >
      ...
      <div class="carousel-inner py-3" style="height: 70vh;" >  
        ...
        <div id="image-carousel" v-for="image in product.images" v-bind:class="`carousel-item ` + (image == product.images[0] ? 'active' : '')">
          <img v-bind:class="`d-block img-fluid` + (image == product.images[0] ? 'active' : '')" style="margin-left: auto; margin-right: auto; max-width: 400px; height: auto; width: 100%;" v-bind:src="`http://localhost:8081/public/products/` + image" alt="Image">
        </div>
...
export default {
  ...
  data () {
    ...
    methods: {
      getProducts(){
        fetch('http://localhost:8081/product/' + this.Id)
        .then((res) => {
